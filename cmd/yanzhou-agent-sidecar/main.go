...
func run(ctx context.Context, args []string, stdin io.Reader, stdout io.Writer) error {
	...
	for {
		...
		if frame.Kind == yanzhouprotocol.KindRunCancel {
			...
			var cancelErr error
			_, planRun := planRunIDs.Load(runID)
			if planRun {
			...
			featureNegotiated := writingHarnessNegotiated
			if planRun {
				featureNegotiated = planModeNegotiated
			}
			if !featureNegotiated || decodeErr != nil || cancelErr != nil {
		...
		if frame.Kind == yanzhouprotocol.KindRunStart {
			planMode, runID, decodeErr := runStartMetadata(frame.Payload)
			switch {
			case decodeErr != nil:
			...
			case planMode && planModeNegotiated:
				frame.RunID = runID
				planRunIDs.Store(runID, struct{}{})
				writingRuns.Add(1)
				go func(input yanzhouprotocol.Envelope) {
					defer writingRuns.Done()
					defer func() {
						if recover() != nil {
							_ = planRuntime.FailPanic(ctx, input.RunID, stdout)
						}
						if !planRuntime.HasRun(input.RunID) {
							planRunIDs.Delete(input.RunID)
						}
					}()
			...
			case !planMode && writingHarnessNegotiated:
				...
				go func(input yanzhouprotocol.Envelope) {
					...
					defer func() {
						if recover() != nil {
							_ = writingRuntime.FailPanic(ctx, input, stdout)
						}
					}()
		...
		resumeRunID, decodeErr := runResumeRunID(frame.Payload)
		if decodeErr != nil {
			if err := writeRuntimeError(stdout, frame, "plan_frame_rejected"); err != nil {
				return err
			}
			continue
		}
		frame.RunID = resumeRunID
		writingRuns.Add(1)
		go func(input yanzhouprotocol.Envelope) {
			defer writingRuns.Done()
			defer func() {
...
func runStartPlanMode(payload json.RawMessage) (bool, error) {
	planMode, _, err := runStartMetadata(payload)
	return planMode, err
}

func runStartMetadata(payload json.RawMessage) (bool, string, error) {
	var value struct {
		PlanMode *bool  `json:"planMode"`
		RunID    string `json:"runId"`
	}
	decoder := json.NewDecoder(bytes.NewReader(payload))
	if err := decoder.Decode(&value); err != nil || value.PlanMode == nil || strings.TrimSpace(value.RunID) == "" || len(value.RunID) > 128 {
		return false, "", errors.New("run.start metadata is invalid")
	}
	return *value.PlanMode, value.RunID, nil
}

func runResumeRunID(payload json.RawMessage) (string, error) {
	var value struct {
		RunID string `json:"runId"`
	}
	if err := json.Unmarshal(payload, &value); err != nil || strings.TrimSpace(value.RunID) == "" || len(value.RunID) > 128 {
		return "", errors.New("run.resume runId is invalid")
	}
	return value.RunID, nil
}
