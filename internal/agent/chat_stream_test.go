...
func TestProcessStreamingEventKeepsFirstCandidateWhenTurnReadinessRunsAhead(t *testing.T) {
	reader, writer := schema.Pipe[*schema.Message](1)
	writer.Send(&schema.Message{Role: schema.Assistant, Content: "第二段不应覆盖首个候选。"}, nil)
	writer.Close()

	var content strings.Builder
	content.WriteString("首个正文候选。")
	var thinking strings.Builder
	var events []Event
	_, err := processStreamingEvent(
		context.Background(),
	...
	if got := content.String(); got != "首个正文候选。" {
		t.Fatalf("later prose replaced the first candidate: %q", got)
	}
	if got := thinking.String(); got != "第二段不应覆盖首个候选。" {
		t.Fatalf("later prose was not isolated as thinking: %q", got)
	}
...
func TestProcessNonStreamingEventKeepsFirstCandidateWhenTurnReadinessRunsAhead(t *testing.T) {
	var content strings.Builder
	content.WriteString("首个正文候选。")
	var thinking strings.Builder
	var events []Event
	processNonStreamingEvent(
		&adk.MessageVariant{Message: schema.AssistantMessage("第二段不应覆盖首个候选。", nil)},
		&content,
		&thinking,
		0,
		agentEventMetadata{AgentKind: AgentKindInteractiveStory},
