...
func TestBookConceiveEmitsCompleteCanonicalConceptionArtifact(t *testing.T) {
	...
	request["idempotencyKey"] = "idem-" + runID
	request["capabilityId"] = "book.conceive"
	request["harnessProfile"] = "novel-standard"
	request["effectiveModelProfile"].(map[string]any)["model"] = "deepseek-v4-pro"
	...
	request["userIntent"] = "完整起笔对话：" + strings.Repeat("作者确认凌晨四点营业。", 600)
	...
	if err := runtime.HandleFrame(context.Background(), yanzhouprotocol.Envelope{
		...
		RequestID: "request-" + runID, Payload: payload,
	}, &output); err != nil {
		t.Fatal(err)
	}
	...
	if calls != 2 || !equalAnySlice(artifactKinds, []any{"conception", "review", "report"}) {
