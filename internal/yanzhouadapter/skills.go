...
func validateRunSkillSnapshot(runID string, selected []string, snapshot *SkillSnapshot) error {
	if len(selected) == 0 {
		if snapshot != nil {
			return fmt.Errorf("Skill snapshot requires an explicit selection")
		}
		return nil
	}
	if snapshot == nil || snapshot.SchemaVersion != "1" || snapshot.RunID != runID || len(snapshot.Skills) != len(selected) {
		return fmt.Errorf("explicit Skill requires a matching loaded snapshot")
	}
	receipts := make(map[string]SkillLoadReceipt, len(snapshot.Skills))
	for _, receipt := range snapshot.Skills {
		if receipt.Validate() != nil || receipts[receipt.ID].ID != "" {
			return fmt.Errorf("Skill snapshot receipt is invalid")
		}
		receipts[receipt.ID] = receipt
	}
	seen := map[string]bool{}
	for _, id := range selected {
		if !validPlanSchemaID(id) || seen[id] || receipts[id].ID == "" {
			return fmt.Errorf("Skill snapshot does not match selected Skills")
		}
		seen[id] = true
	}
	return nil
