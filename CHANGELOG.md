...
- 用户影响：完成 Phase 1 工程集成与退出门禁，验证新建、canonical `.denova` 与 legacy-only `.nova` workspace 的只读打开和迁移恢复，确认可删除 SQLite/FTS 投影能够从正式文件等价重建、作者外部 Markdown 修改保持事实权威，并回归作品质量页面、四个冻结 API、双语共享菜单和写作/游戏模式保持。本次不新增产品操作、配置项、API、依赖或 Phase 2 能力，也不作 Harness 产品质量、Beta、发布或出版就绪声明。
- User impact: completed the Phase 1 engineering integration and exit gates for read-only opening and migration recovery across fresh, canonical `.denova`, and legacy-only `.nova` workspaces; equivalent SQLite/FTS rebuilds from formal files; authoritative external Markdown edits; and regression coverage for the Project Quality page, four frozen APIs, bilingual shared navigation, and writing/game mode preservation. This adds no product action, configuration, API, dependency, or Phase 2 capability and makes no Harness product-quality, Beta, release, or publication-readiness claim.
- 工程影响：新增 Workspace/migration、Projection/外部修改、authority/version 的跨边界集成测试和一个使用生产重试语义的 Quality 错误态回归测试；版本矩阵精确覆盖正式/审计记录的包含与 index/cache/run/checkpoint/session/migration 临时状态的排除，Candidate 与未确认 Preference 默认不进入正式正文或 Projection 来源。`workflow.input.invalidated` 仅验证已冻结的 P1 事件与 SSE transport 合同，不实现或声称 P2 persistent Run、自动失效编排或 Context Pack Builder。生产代码、Accepted ADR、normative schema、`go.mod`、lockfile 和前端 manifest 均未修改。
- Engineering impact: added cross-boundary integration tests for Workspace/migration, Projection/external edits, and authority/version behavior, plus a Project Quality error-state regression using production retry semantics. The version matrix explicitly includes formal/audit records and excludes disposable index/cache/run/checkpoint/session/migration state; Candidate and unconfirmed Preference inputs do not become formal prose or default Projection sources. `workflow.input.invalidated` validates only the frozen P1 event and SSE transport contract and neither implements nor claims a P2 persistent Run, automatic invalidation orchestration, or Context Pack Builder. Production code, Accepted ADRs, normative schemas, `go.mod`, lockfiles, and frontend manifests are unchanged.

- 用户影响：新增共享一级入口“作品质量”与只读作品质量主页/规划中心，在写作和游戏模式中都可查看作品概览、三类只读 Profile 参考、作者可读 QualitySpec、workspace compatibility 和零写入 migration preview。进入、退出或重复点击该共享入口不会自动切换写作/游戏内容模式；页面不安装或选择 Profile，不应用迁移，也不提供 Quality Run、Candidate、Review、Preference 或 Finalization 操作。
- User impact: added the shared top-level “Project Quality” entry and a read-only quality home/planning center for both writing and game modes. It presents the project overview, three read-only Profile references, an author-readable QualitySpec, workspace compatibility, and a zero-write migration preview. Entering, leaving, or toggling the shared entry does not switch the writing/game content mode; the page does not install or select Profiles, apply migrations, or expose Quality Run, Candidate, Review, Preference, or Finalization actions.
- 工程影响：新增四个冻结 Quality API 的前端 DTO/严格 v1 guard、TanStack Query 边界、独立 lazy Quality route、双语文案与 light/dark、宽窄屏、长文本、空数据、未知/更新/畸形合同和 404/409/500/网络失败状态保护。实现未修改 Go 生产代码、API 路由、Accepted ADR、normative schema、配置项或依赖清单。
- Engineering impact: added frontend DTOs and strict v1 guards for the four frozen Quality APIs, a TanStack Query boundary, an independently lazy Quality route, bilingual copy, and bounded handling for light/dark themes, wide/narrow viewports, long text, empty data, unknown/newer/malformed contracts, and 404/409/500/network failures. The implementation changes no Go production code, API route, Accepted ADR, normative schema, configuration option, or dependency manifest.

- 用户影响：新增四个只读/零写入的 Quality API：`GET /api/quality/profiles`、`GET /api/quality/profiles/:profile_id`、`GET /api/quality/project` 与 `POST /api/quality/project/migration-preview`。它们只检查内置 Profile 目录、当前项目和迁移预览；预览不写入任何内容，内置 Profile 不会安装为 workspace 默认值或作者真源。本次没有新增 UI、Quality Run/start/action 接口、运行时/状态机、Run 仓储、模型调用、Preference 写入、Finalization 或正式内容写入，也不作小说质量或发布就绪声明。
- User impact: added four read-only/zero-write Quality APIs: `GET /api/quality/profiles`, `GET /api/quality/profiles/:profile_id`, `GET /api/quality/project`, and `POST /api/quality/project/migration-preview`. They inspect only the bundled Profile catalog, current project, and migration preview; preview writes nothing, and bundled Profiles are neither installed as workspace defaults nor made author truth. This adds no UI, Quality Run/start/action endpoint, runtime/state machine, Run repository, model call, Preference write, finalization, or formal-content write, and makes no fiction-quality or release-readiness claim.
- 工程影响：冻结有界 Quality 事件/SSE 合同，保留生产者原始事件身份并支持重连显示恢复；Agent UI 对未知或不安全数据只投影安全、有界活动。事件与 SSE 不携带正文、提示词、thinking、路径、密钥或其他无界载荷，慢订阅者只会丢显示投递，绝不丢持久状态。
- Engineering impact: froze a bounded Quality event/SSE contract that preserves producer-owned event identity and reconnect display recovery; Agent UI projects unknown or unsafe data only as safe, bounded activity. Events and SSE carry no content, prompt, thinking, path, secret, or other unbounded payload; a slow subscriber can lose display delivery but never durable state.

- 用户影响：完成 P1-T04 的独立记录边界，为 CandidateSet、ReviewIssue 与作者显式确认的 PreferenceMemory 提供可审计文件真源；候选/审稿记录仍不是正式正文或设定真源，偏好记录不能自动改正文、QualitySpec、候选决定或触发 Finalization。本次未接入 API、UI、Agent、Automation 或完整 Harness，因此现有产品交互不变。
- User impact: completed the standalone P1-T04 record boundary for auditable CandidateSet, ReviewIssue, and explicitly author-confirmed PreferenceMemory sources. Candidate/review records remain non-formal Artifacts, and preferences cannot mutate formal content, QualitySpec, CandidateSet decisions, or trigger Finalization. This change adds no API, UI, Agent, Automation, or full Harness integration, so existing product interactions are unchanged.
- 工程影响：新增三份 Accepted v1 schema 的显式注入、Draft 2020-12 严格解码与语义验证；CandidateSet/ReviewIssue 使用 ID 派生的 `.denova/quality/artifacts/**` 文件仓储和 raw-hash/revision CAS，PreferenceMemory 使用有界 `.denova/quality/preferences.jsonl` 完整文件追加、确定性 scope 解析与 correction/revocation 历史。写入复用 Workspace Schema v1 门禁、portable-path、同目录完整暂存、文件 sync、原子发布、父目录 sync 与跨进程锁；未知版本保留原字节只读，迁移仅可预览且在没有 superseding Accepted ADR 时零写入拒绝。
- Engineering impact: added explicit schema-byte injection, strict Draft 2020-12 decoding, and semantic validation for all three Accepted v1 contracts. CandidateSet/ReviewIssue use ID-derived `.denova/quality/artifacts/**` repositories with raw-hash/revision CAS; PreferenceMemory uses bounded whole-file append at `.denova/quality/preferences.jsonl`, deterministic scope resolution, and retained correction/revocation history. Writes reuse the Workspace Schema v1 guard, portable paths, complete sibling staging, file sync, atomic publication, parent sync, and a cross-process lock. Unknown versions retain exact read-only bytes, while migration remains preview-only and refuses all writes without a superseding Accepted ADR.
- 用户影响：新增离线 `denova reindex --workspace <path>`，可完全从正式 workspace 文件和显式批准的 Artifact 路径重建可删除的 `.denova/index.db`，并输出中英文可定位摘要。SQLite 缺失、锁定、损坏、schema 更新、索引不一致或作者外部改稿都不会把数据库提升为真源，也不会阻止作品继续读取和编辑；本次没有 API、UI、产品启动自动重建或 Harness 接入，因此现有产品界面与创作流程不变。
- User impact: added offline `denova reindex --workspace <path>` to rebuild the disposable `.denova/index.db` entirely from formal workspace files and explicitly approved Artifact paths, with a locatable bilingual summary. Missing, locked, corrupt, newer-schema, inconsistent, or externally stale SQLite state never becomes authority and never blocks continued workspace reading/editing. This change adds no API, UI, product-startup auto-rebuild, or Harness integration, so the existing product interface and writing flow are unchanged.
- 工程影响：完成 P1-T03 Projection v1 独立边界，精确锁定 BSD-3-Clause `modernc.org/sqlite v1.54.0` 及其 upstream `go.mod` 匹配的 `modernc.org/libc v1.74.1`，并将既有 `x/sys v0.46.0` tidy 分类债务改为 direct（版本不变）。schema v1 分离稳定 document ID 与 SQLite integer rowid，使用 FTS5 external-content + trigram、同事务 insert/update/delete triggers、每次新 sibling 激活/重建完成/损坏检查的 `rank=1`，以及少于三个 Unicode 字符的精确扫描 fallback。重建以真实 `buildinfo.Version` 和精确 v1 feature 范围执行 Workspace Schema 写门禁，并使用跨进程锁、唯一同目录 sibling、完整 schema/data/hash/FTS/大小复验、关闭连接、source CAS、原子替换和父目录 sync；激活前后出现的 journal/WAL/SHM 会连同主库持久化隔离，非普通或部分隔离失败也不会把已隔离主库回滚为可见。损坏投影只按 rename 隔离后从文件真源重建，从不复制 live SQLite 或反向写入正式文件/Artifact。
- Engineering impact: completed the standalone P1-T03 Projection v1 boundary with exact BSD-3-Clause pins for `modernc.org/sqlite v1.54.0` and the matching `modernc.org/libc v1.74.1` from sqlite's upstream `go.mod`, while correcting the existing `x/sys v0.46.0` tidy classification to direct without changing its version. Schema v1 separates stable document IDs from SQLite integer rowids, uses FTS5 external-content plus trigram, transactional insert/update/delete triggers, exact `rank=1` checks for every fresh sibling activation, rebuild completion, and corruption check, and exact-scan fallback below three Unicode characters. Rebuild enforces the Workspace Schema writer gate with the real `buildinfo.Version` and exact v1 feature ranges, then uses a cross-process lock, a unique same-directory sibling, complete schema/data/hash/FTS/size revalidation, connection close, source CAS, atomic replacement, and parent sync. Journal/WAL/SHM entries appearing before or after activation are durably quarantined with the main database; a non-regular sidecar or partial quarantine failure never rolls the already isolated main back into visibility. Unsafe projections are rebuilt from files, never copied live or written back into formal files/Artifacts. No P1-T04, P1-T07, API, UI, App service, Agent, Automation, Harness, Author Finalization, Tauri, vector search, or custom tokenizer is included.
- 用户影响：P1-T02 Workspace Schema v1 工程实现已完成。在保持只读检查和迁移预览零写入的前提下，新 workspace 可安全发布 `not_required` marker；现有 `.denova` 与 legacy-only `.nova` workspace 只有在作者显式授权后才会备份、完整暂存、切换、经正常 reader 复验并生成可核验回执。本次未接入 API、UI 或 Workspace 启动流程，因此当前产品界面和创作流程不变。
- User impact: P1-T02 Workspace Schema v1 is engineering-complete. Read-only inspection and migration preview remain zero-write; a new workspace can safely publish a `not_required` marker, while existing `.denova` and legacy-only `.nova` workspaces require explicit author authorization before backup, complete staging, switching, normal-reader verification, and an auditable receipt. This change does not integrate API, UI, or workspace-startup flows, so the current product interface and writing workflow are unchanged.
- 工程影响：新增不依赖 API/UI 的显式迁移执行边界，使用最窄 writer-lease 接口，将 canonical workspace/root 身份、migration ID、完整 preview 与 source hash 集合、目标 schema/features 及作者确认证据绑定。版本化状态、备份/stage manifest、marker 和 receipt 都按同目录临时文件、文件 sync、rename 与父目录 sync 的顺序持久化，并支持按 hash 幂等续跑、故障对账、显式回滚与外部变更冲突保护。原子性仅声明为同文件系统中的单一 namespace entry；不声称跨文件系统或 filesystem+Git ACID，不删除唯一 verified backup，不双写保留的 `.nova`，也未实现 P1-T03、Author Finalization、SQLite/FTS、Agent、Automation 或产品 Harness，不作小说质量或可发布声明。
- Engineering impact: added an API/UI-independent migration executor behind the narrow shared writer-lease interface, binding canonical workspace/root identity, migration ID, the complete preview and source-hash set, target schema/features, and author-confirmation evidence. Versioned state, backup/stage manifests, markers, and receipts use same-directory temporary files followed by file sync, rename, and parent-directory sync, with hash-idempotent resume, crash reconciliation, explicit rollback, and external-change conflict protection. Atomicity is claimed only for one namespace entry on the same filesystem; this is not cross-filesystem or filesystem-plus-Git ACID, the sole verified backup is retained, retained `.nova` is never dual-written, and this change does not implement P1-T03, Author Finalization, SQLite/FTS, Agent, Automation, or the product Harness, nor does it claim fiction-quality improvement or release readiness.
...
- 调整霞萍发现工件的明确大小上限至每项 2 MiB，使完整实时写作候选池可以发布而不截断其元数据证据；上限仍受 Schema 与原子发布事务约束。
...
- Added Quality Harness implementation plans covering architecture evolution, overall and Phase 0 development, requirements traceability, risk decisions, and validation/release gates.
...
- 修复虾评 schema 的嵌套回归覆盖与工件字节边界：测试现在逐项篡改页面、失败、候选、提案、簇、短名单和词典路径；所有 provenance `max_bytes` 固定为 262144。
- Fixed Xiaping schema nested-regression coverage and artifact byte boundary: tests now mutate page, failure, candidate, proposal, cluster, shortlist, and lexicon paths individually; every provenance `max_bytes` is fixed at 262144.
