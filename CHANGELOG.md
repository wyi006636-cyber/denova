...
- 用户影响：完成 P1-T02A 的 Workspace Schema v1 安全读取、精确路径分类、兼容性拒写与零写入迁移预览；现有 current/legacy 作品在 marker 缺失、损坏或更新时仍可安全打开，预览不会创建、复制、改名、删除、采用或迁移任何作品文件。P1-T02B 的备份、暂存、切换、回执、续跑、回滚与恢复执行器尚未实现，P1-T02 尚未整体完成。
- User impact: completed the P1-T02A Workspace Schema v1 safe reader, exact path classification, compatibility mutation guard, and zero-write migration preview; existing current and legacy works remain safely openable when the marker is missing, malformed, or newer, and preview never creates, copies, renames, deletes, adopts, or migrates work files. P1-T02B backup, staging, switching, receipts, resume, rollback, and recovery execution remain unimplemented, so P1-T02 is not complete overall.
- 工程影响：新增固定 active-root 的只读 adapter、严格 SemVer 2.0.0 writer/feature 合同、原始 marker 字节保留、canonical containment 与跨平台路径 preflight、确定性 SHA-256 preview manifest，以及 default-include 的版本排除和 restore 保留策略；仅将模块图中既有且已锁定的 `Masterminds/semver v3.4.0` 标记为 direct dependency，不升级版本、不修改 `go.sum`，也未接入 P1-T03、Author Finalization、API、UI、Agent、Automation、SQLite/FTS 或产品 Harness。
- Engineering impact: added a pinned-active-root read-only adapter, strict SemVer 2.0.0 writer/feature contracts, exact marker-byte preservation, canonical containment and cross-platform path preflight, deterministic SHA-256 preview manifests, plus default-include version exclusions and restore preservation; the already locked `Masterminds/semver v3.4.0` module-graph dependency is only classified as direct, with no version upgrade or `go.sum` change, and this does not integrate P1-T03, Author Finalization, API, UI, Agent, Automation, SQLite/FTS, or the product Harness.
